...
def extract_pdf(
    ...
    *,
    ...
    try:
        ...
        # 3) Merge output deterministically: one final text block per page.
        low_page_set = set(low_pages)
        for i in range(n_proc):
            merged_txt = page_texts[i]
            source = "TEXT"

            if i in low_page_set and ocr_pages and i < len(ocr_pages):
